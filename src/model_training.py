...
def filter_gaussian(X): 
    """
    Chooses most gaussian looking distributions (hand picked using EDA) 
    (doesn't improve any score)
    """
...
def drop_outliers(X, y, lim=0.5): 
    """
    Drops data points that have too large NO168.mean scores
    (doesn't improve any score)
    """
...
def top_corr_features(X): 
    """
    Excludes the features with the lowest correlation  
    (doesn't improve any score)
    """
    # all features with correlation of over 0.2 
    top_corr = ['Glob.mean', 'Glob.std', 'NET.mean', 'NET.std', 'O3168.mean',
       ...
       'UV_B.std']
    # all features with correlation over 0.3
    top_top_corr = ['Glob.mean','PAR.mean','RHIRGA168.mean','RHIRGA336.mean','RHIRGA42.mean','RHIRGA42.std','RHIRGA504.mean','RHIRGA672.mean','RHIRGA84.mean']
    X = X[top_corr]
    return X

def polynomial(X, deg=2, interaction_only=True): 
    """
    Adds polynomial features 
    (doesn't improve the best scores and is very computationally expensive)
    """
    X = PolynomialFeatures(deg, interaction_only=interaction_only).fit_transform(X)
    return X
...
def perplexity_scorer(estimator, X, y): 
    ...
    Returns the perplexity score for a given estimator
    """
...
def binary_accuracy_scorer(estimator, X, y):
    """
    Returns the binary accuracy (for predicting event vs non-event)
    """
...
# Scores returned by cross-validation
scoring = {
...
def test_model(X, y, pipeline, n=0):
    ...
    cv = StratifiedKFold(n_splits=10, shuffle=True) # cross-validation splits

    # probability calibration, results in no significant difference
...
def return_pipeline(n): 
    ...
    No significant score improvement with any combination or model
    """
...
if __name__ == "__main__":
    ...
    # Processing training and test data into usable formats
    ...
    # All pipelines used in ensembles
    ...
    s.to_csv(path / 'triple_scores.csv')
